var checkPermissionUtil = require('../../utils/check-permission-util.js');//调用授权
var baiduTokenUtil = require('../../utils/baidu-token-util.js');//调用token
...
Page({
    ...
    onBindCamera: function () {
        ...
        try {
            checkPermissionUtil.checkPermission('scope.camera').then(function (res) {
                    ...
                    wx.navigateTo({
                        url: 'camera/camera',//如果授权成功即可跳转 首次允许或者曾允许，否则需要下一次触发拍照
                    });
                }, function (err) {
    ...
    async startRecordHandle() {
        ...
        try {
            await checkPermissionUtil.checkPermission('scope.record').then(function (res) {
                    console.log('检测权限结果：' + res);
                    that.record();//授权后调用之后的record()准备录音
                }, function (err) {
                    console.error('检测权限结果失败：' + err);
                    wx.showToast({
                        title: '授权失败，无法使用该功能~',
    ...
    record: function () {
        ...
        if (this.data.canRecordStart) {
            recorderManager.start(options);//录音管理器开始工作
            this.isSpeaking = true;//设置录音状态变量
        }
